...
function ResetPassword() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      const isSetupLink = window.location.hash.includes("type=recovery") || window.location.hash.includes("type=invite") || new URLSearchParams(window.location.search).has("code");
      setValid(Boolean(data.session) || isSetupLink);
    };
    void checkSession();
  }, []);
