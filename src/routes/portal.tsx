...
function Portal() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [appointmentsResult, formsResult, paymentsResult, servicesResult] = await Promise.all([
      ...
      const queryError = appointmentsResult.error ?? formsResult.error ?? paymentsResult.error ?? servicesResult.error;
      if (queryError) throw queryError;
      return { patient, appointments: appointmentsResult.data ?? [], forms: formsResult.data ?? [], payments: paymentsResult.data ?? [], services: servicesResult.data ?? [] };
    },
  });
