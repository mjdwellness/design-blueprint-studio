...
export function DatabaseAdminPage({ kind }: { kind: Kind }) {
  ...
  const mutation=useMutation({mutationFn:async(values:Record<string,string>)=>{
    ...
    const payload=config.payload(values); const query=editing?.id?supabase.from(config.table).update(payload).eq("id",editing.id):supabase.from(config.table).insert(payload as never); const {error}=await query; if(error) throw error;
